...
const getAvailableSlots = () => {
  ...
  if (typeof window === "undefined") return availableSlots

  const savedSchedule = localStorage.getItem("doctorSchedule")
  if (!savedSchedule) return availableSlots
  ...
  for (let i = 0; i < 30; i++) {
    ...
    if (schedule.mode === "weekly") {
      if (schedule.weeklyDays.includes(dayName)) {
        const timeSlots: string[] = []
        const startHour = Number.parseInt(schedule.workingHours.start)
        const endHour = Number.parseInt(schedule.workingHours.end)

        for (let hour = startHour; hour < endHour; hour++) {
          timeSlots.push(`${String(hour).padStart(2, "0")}:00 AM`)
          if (hour < 12) timeSlots.push(`${String(hour).padStart(2, "0")}:30 AM`)
          else if (hour === 12) timeSlots.push("12:30 PM")
          else timeSlots.push(`${String(hour - 12).padStart(2, "0")}:30 PM`)
        }
        slots[dateStr] = timeSlots
      }
    } else if (schedule.mode === "specific" && schedule.specificDate === dateStr) {
      slots[dateStr] = schedule.specificSlots
    }
  }

  return Object.keys(slots).length > 0 ? slots : availableSlots
}
...
export default function Home() {
  ...
  >([])
  const [dynamicSlots, setDynamicSlots] = useState(() => getAvailableSlots())
  ...
  useEffect(() => {
    const handleStorageChange = () => {
      setDynamicSlots(getAvailableSlots())
    }
  ...
  const addAppointment = async (appointment: any) => {
    try {
      ...
      addNotification("success", `Appointment booked for ${appointment.patientName}!`)
    } catch (error) {
      console.error("[Landing] Error updating appointments after booking:", error)
    }
  }
      ...
      <main className="overflow-hidden">
        ...
        <header className="sticky top-0 z-50 bg-white/95 backdrop-blur supports-[backdrop-filter]:bg-white/60 border-b">
          <div className="px-4 py-4 md:py-6 max-w-7xl mx-auto flex justify-between items-center">
            <div className="text-2xl font-bold text-teal-600">Dr. Fatima Bennani</div>
        ...
        <ContactForm language={language} onNotification={addNotification} />
          ...
          <div className="mx-auto max-w-6xl">
            ...
            <div className="border-t border-slate-700 pt-8 flex flex-col md:flex-row justify-between items-center gap-4">
              ...
              <Button
                ...
                className="border-teal-600 text-teal-400 hover:bg-teal-600/10 bg-slate-800 border"
              >
                {t.nav.admin}
              </Button>
            </div>
